...
// [T36] サムネイルは本文ブロック列(diaryBlocks)から完全に独立して管理する。
// 以前は「先頭の写真ブロックを自動採用」しており、その写真がサムネと本文の両方に
// 表示される二重表示バグの原因だった。diaryThumbData は base64(未アップロード) または
// S3 URL(アップロード済み/編集時)を保持し、null なら「サムネなし」。
var diaryThumbData = null;
...
// ========== サムネイル専用ブロック（[T36] 本文ブロックとは独立） ==========

/** サムネスロットの描画: 未設定なら「サムネイルを追加」ボタン、設定済みならプレビュー+削除ボタン */
function diaryRenderThumbSlot() {
  var body = document.getElementById('diaryThumbSlotBody');
  if (!body) return;
  if (diaryThumbData) {
    var src = safeImageSrc(diaryThumbData);
    body.innerHTML =
      '<div class="dbe-thumb-slot-preview">'
      + (src ? '<img src="' + src + '" alt="">' : '')
      + '<button type="button" class="dbe-thumb-slot-remove" onclick="diaryRemoveThumb()" aria-label="サムネイルを削除"><i class="ph-bold ph-x"></i></button>'
      + '</div>';
  } else {
    body.innerHTML =
      '<button type="button" class="dbe-thumb-slot-add" onclick="diaryPickThumbPhoto()"><i class="ph-bold ph-image"></i> サムネイルを追加</button>';
  }
}

function diaryPickThumbPhoto() {
  var input = document.getElementById('diaryThumbPhotoInput');
  if (input) { input.value = ''; input.click(); }
}

function diaryThumbPhotoFileSelected(event) {
...
function diaryRestoreDraft() {
  ...
  diaryThumbData = draft.thumbData || null;
...
async function submitDiary() {
  ...
  try {
    ...
    // 3. サムネイル = 専用スロット(diaryThumbData)のみ。[T36] 本文の写真ブロックからは
    //    自動採用しない(それが本文写真との二重表示バグの原因だった)。未アップロード(base64)
    //    ならここでS3へアップロードし、既にS3 URLならそのまま使う。
    var finalCatchImageUrl = null;
    if (diaryThumbData) {
      finalCatchImageUrl = diaryThumbData.indexOf('data:') === 0
        ? await uploadImageToS3(diaryThumbData, 'diary')
        : diaryThumbData;
    }
...
function editDiary(postId) {
  ...
  // [T36] サムネイルは catchImageUrl(/旧CATCH_IMG) をそのまま専用スロットへ読み込むだけ。
  // 本文ブロック(diaryBlocks)には一切手を加えない — 本文に同じ写真があっても手を加えず、
  // 旧記事でcatchImageUrlが無い場合もサムネは「なし」から始める(本文から自動採用しない)。
  diaryThumbData = editCatchImg || null;
